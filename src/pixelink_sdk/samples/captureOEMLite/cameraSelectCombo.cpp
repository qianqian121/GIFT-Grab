...
static gboolean rebuildCameraSelectCombo (gpointer pData)
{
	...
	bool changingCamerass = combo->m_selectedCamera != combo->m_requestedCamera;
	...
	if (!changingCamerass && combo->m_selectedCamera != 0)
	{
	...
	//      update all of the other controls.
	 if (changingCamerass)
	 {
	     combo->m_cameraChangeInProgress = true;
	     previewObject->greyAll();
    	 exposureObject->greyAll();
    	 pixelFormatObject->greyAll();
    	 pixelAddressObject->greyAll();
    	 roiObject->greyAll();
    	 frameRateObject->greyAll();
    	 gainObject->greyAll();
         focusObject->greyAll();
    	 whiteBalanceObject->greyAll();
    	 imageFlipObject->greyAll();
    	 imageCaptureObject->greyAll();
	     if (combo->m_selectedCamera != 0)
	     {
	    	 previewObject->activatePlay();
	    	 exposureObject->initialize();
	    	 pixelFormatObject->initialize();
	    	 pixelAddressObject->initialize();
	    	 roiObject->initialize();
	    	 frameRateObject->initialize();
	    	 gainObject->initialize();
             focusObject->initialize();
	    	 whiteBalanceObject->initialize();
	    	 imageFlipObject->initialize();
	     }
	     combo->m_cameraChangeInProgress = false;
	 }
